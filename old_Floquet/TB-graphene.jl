...
Q = 0.5  # Example value, replace with actual value
W = 0.25  # Example value, replace with actual value
F = 5.0  # Example value, replace with actual value
Nk =40 # Example value, replace with actual value
h_dim=2 # TB-Hamiltonian dimension

kpt_with_border=false #include or not k-points at the border of the BZ
...
if kpt_with_border
  ...
  full_kpts=range(-pi/2.0,pi/2.0,Nk+1)
  kpts=zeros(Float64,Nk)
  for ik in 1:Nk
    kpts[ik]=full_kpts[ik]
  end
end
