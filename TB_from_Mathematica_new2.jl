...
function calculate_and_plot_IH()
    global Nm, W, F, Q
    ...
    # Calculate IH for different N0 values and Nk values
    N0_values = 1:30
    Nk_values = [10, 20, 30, 50]
    
    results = Dict{Int, Vector{Complex{Float64}}}()
    
    for Nk in Nk_values
        println("Calculating for Nk = $Nk...")
        ih_values = Complex{Float64}[]
        for N0 in N0_values
            println("  N0 = $N0")
            ih_val = IH(N0, Nk)
            push!(ih_values, ih_val)
        end
        results[Nk] = ih_values
    end
    
    # Plot real part
    p1 = plot(title="Real part of IH(N0)", 
              xlabel="N0", 
              ylabel="Re(IH)",
              legend=:topleft)
    
    for Nk in Nk_values
        plot!(p1, N0_values, real.(results[Nk]), 
              label="Nk=$Nk", 
              marker=:circle,
              linewidth=2)
    end
    ...
    # Plot imaginary part
    p2 = plot(title="Imaginary part of IH(N0)", 
              xlabel="N0", 
              ylabel="Im(IH)",
              legend=:topleft)
    
    for Nk in Nk_values
        plot!(p2, N0_values, imag.(results[Nk]), 
              label="Nk=$Nk", 
              marker=:circle,
              linewidth=2)
    end
    ...
    # Plot absolute value
    p3 = plot(title="Absolute value of IH(N0)", 
              xlabel="N0", 
              ylabel="|IH|",
              legend=:topleft)
    
    for Nk in Nk_values
        plot!(p3, N0_values, abs.(results[Nk]), 
              label="Nk=$Nk", 
              marker=:circle,
              linewidth=2)
    end
    ...
    # Save and display
    savefig(p_final, "IH_vs_N0.png")
    display(p_final)
    
    # Print results for N0 = 30 with highest Nk
    println("\nResults for N0 = 30 (highest accuracy):")
    println("Nk = 50: IH(30) = $(results[50][end])")
    
    return results
end

# Run the calculation
results = calculate_and_plot_IH()
